        ...
        async () => {
          setTimeout(async () => {
            ...
            if (!(isCordova && isAndroid)) {
              ...
              chooser.getFile().then(
                async (_files) => {
                  await this.uiAlert.showLoadingSpinner();

                  for (const file of _files) {
                    try {
                      await this.filePath.resolveNativePath(file.uri).then(
                        async (path) => {
                          if (
                            path &&
                            ...
                            if (newPath.lastIndexOf('/Download/') > -1) {
                              ...
                              pathFromDownload =
                                pathFromDownload +
                                decodedURI.substring(
                                  decodedURI.lastIndexOf('/Download/')
                                );
                            ...
                            await this.uiFileHelper
                              .copyFileWithSpecificName(importPath)
                              .then(
                                async (_fullPath) => {
