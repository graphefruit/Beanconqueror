...
export class TimemoreBasicScale extends BluetoothScale {
  public static DEVICE_NAME = 'basic3';
  ...
  constructor(data: PeripheralData, type: ScaleType) {
    super(data, type);
    this.logger = new Logger('Timemore Basic Scale');
    this.connect();
  }
  ...
  public static test(device: any): boolean {
    return (
      ...
      (device.name.toLowerCase().includes('basic3') ||
        ...
        (device.name.toLowerCase().includes('timemore') &&
          device.name.toLowerCase().includes('basic')))
    );
  }
  ...
  private write(_bytes: Uint8Array): Promise<boolean> {
    return new Promise((resolve) => {
      ble.writeWithoutResponse(
        this.device_id,
        TimemoreBasicScale.SERVICE_UUID,
        TimemoreBasicScale.CMD_UUID,
        _bytes.buffer,
        (e: any) => {
          resolve(true);
        },
  ...
  private async parseStatusUpdate(rawStatus: Uint8Array) {
    ...
    // Validate CRC
    const receivedCrc = (rawStatus[6 + dataLen] << 8) | rawStatus[7 + dataLen];
    const calculatedCrc = this.crc16Ibm(rawStatus.slice(0, 6 + dataLen));
    if (receivedCrc !== calculatedCrc) {
      this.logger.log('CRC validation failed');
      return;
    }
